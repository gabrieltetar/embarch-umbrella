...
//! Windows service (design.md §3 decision 37).
//!
...
pub fn elevated_script(win_install: &str, win_built: &str, win_log: &str, service: &str) -> String {
    format!(
        "# Generated by `embarch deploy-core` (embarch-umbrella/design.md §3\n\
         # decision 37). Runs elevated; logs each step from *inside* the\n\
...
/// went stale twice. Umbrella has no dependency on the crate (design.md §1 —
/// it orchestrates, it does not link), so reading the source is the honest
